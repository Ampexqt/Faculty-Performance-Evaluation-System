    ...
    try {
        ...
        const query = `
            SELECT 
                ...
                COALESCE(
                    ROUND(
                        (COUNT(DISTINCT CASE WHEN se.id IS NOT NULL THEN CONCAT(s.id, '-', fa.id) END) / 
                        NULLIF(COUNT(DISTINCT CONCAT(s.id, '-', fa.id)), 0)) * 100, 
                        0
                    ), 
                    0
            ...
            LEFT JOIN students s ON s.section = fa.section AND s.status = 'active'
            LEFT JOIN student_evaluations se ON se.faculty_id = f.id AND se.student_id = s.id
            WHERE f.college_id = ? 
                AND f.status = 'active'
    ...
    try {
        ...
        const [evaluations] = await promisePool.query(`
            SELECT 
                ...
                s.subject_code,
                s.subject_name,
                SUBSTRING(fa.section, LOCATE('-', fa.section) - 1, 1) as year_level,
            ...
            LEFT JOIN students st ON st.section = fa.section AND st.status = 'active'
            LEFT JOIN student_evaluations se ON se.faculty_id = fa.faculty_id 
                AND se.student_id = st.id 
                AND se.subject_id = fa.subject_id
